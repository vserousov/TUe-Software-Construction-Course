...
public class KakuroCombinationGeneratorTest {
    ...
    public void setUp() {
        ...
        checker = new Checker();
        PushPullAdapter checkerAdapter = new PushPullAdapter(checker, true);
        instance.setObserver(checkerAdapter);
    }
    ...
    public void checkGenerator(int s, int n, int expCount) {
        ...
        System.out.println("  should generate " + expCount
                + " combination" + ((expCount == 1) ? "" : "s"));
        checker.set(s, n);
        instance.generate(s, n);
        if (expCount != checker.count) {
            System.out.println("  but generated " + checker.count
                    + " combination" + ((checker.count == 1) ? "" : "s"));
        }
